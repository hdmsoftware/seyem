    ...
    return {
        ...
        //    console.log(request)
            ...
            if (regimen.medicationlist.length === 0) {
                res.send(400, {'message': "Medication list cannot be empty!"});
            }

            var newRegimen = new Regimen({name: regimen.name , medicationlist: regimen.medicationlist});

            newRegimen.save(function (err, regimen1) {
                if (err){
                    res.send(500, {'message': err});
                }
                res.json({ 'message': 'Regimen was successfully created!'});
            });
        },
